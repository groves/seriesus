...
(function($) {
    function Plot(target, data_, options_, plugins) {
        ...
        var series = [],
            options = {
                ...
                xaxis: {
                    ...
                    labelHeight: null,
                    grid: {
                      length: "full"// "full" for the plot size, a number for a specific length, or 0 for no grid on this axis
                    },
                ...
                grid: {
                    ...
                    borderMode: null, // null or "onAxes" to draw borders on sides that have ticks
        ...
        function drawGrid() {
            ...
            if (options.xaxis.grid.length) {
                var full = (options.xaxis.grid.length == "full" || options.xaxis.grid.length >= plotHeight);
                var moveY = full ? 0 : plotHeight;
                var lineY = full ? plotHeight : plotHeight - options.xaxis.grid.length;
                for (i = 0; i < axis.ticks.length; ++i) {
                    v = axis.ticks[i].v;
                    if (v <= axis.min || v >= axes.xaxis.max)
                        continue;   // skip those lying on the axes

                    ctx.moveTo(Math.floor(axis.p2c(v)) + ctx.lineWidth/2, moveY);
                    ctx.lineTo(Math.floor(axis.p2c(v)) + ctx.lineWidth/2, lineY);
                }
            }

            axis = axes.yaxis;
            if (options.yaxis.grid.length) {
                var lineX = (options.yaxis.grid.length == "full" || options.yaxis.grid.length >= plotWidth) ? plotWidth : options.yaxis.grid.length;
                for (i = 0; i < axis.ticks.length; ++i) {
                    v = axis.ticks[i].v;
                    ...
                    ctx.lineTo(lineX, Math.floor(axis.p2c(v)) + ctx.lineWidth/2);
                }
            }

            axis = axes.x2axis;
            if (options.x2axis.grid.length) {
                var lineY = (options.xaxis.grid.length == "full" || options.xaxis.grid.length >= plotHeight) ? plotHeight : options.xaxis.grid.length;
                for (i = 0; i < axis.ticks.length; ++i) {
                    v = axis.ticks[i].v;
                    if (v <= axis.min || v >= axis.max)
                        continue;
            ...
            axis = axes.y2axis;
            if (options.y2axis.grid.length) {
                var lineX = (options.y2axis.grid.length == "full" || options.y2axis.grid.length >= plotWidth) ? 0 : plotWidth - options.y2axis.grid.length;
                for (i = 0; i < axis.ticks.length; ++i) {
                    v = axis.ticks[i].v;
                    if (v <= axis.min || v >= axis.max)
                        continue;

                    ctx.moveTo(plotWidth, Math.floor(axis.p2c(v)) + ctx.lineWidth/2);
                    ctx.lineTo(lineX, Math.floor(axis.p2c(v)) + ctx.lineWidth/2);
                }
            }
            ...
            if (options.grid.borderWidth) {
                ...
                ctx.lineWidth = bw;
                var left = 0 - bw/2;
                var right = plotWidth + bw/2;
                var top = 0 - bw/2;
                var bottom = plotHeight + bw/2;
                ctx.strokeStyle = options.grid.borderColor;
                var all = options.grid.borderMode != "onAxes";
                if (all || axes.xaxis.ticks.length) {
                    ctx.moveTo(left - bw/2, bottom);
                    ctx.lineTo(right + bw/2, bottom);
                }
                if (all || axes.yaxis.ticks.length) {
                    ctx.moveTo(left, top - bw/2);
                    ctx.lineTo(left, bottom + bw/2);
                }
                if (all || axes.x2axis.ticks.length) {
                    ctx.moveTo(left - bw/2, top);
                    ctx.lineTo(right + bw/2, top);
                }
                if (all || axes.y2axis.ticks.length) {
                    ctx.moveTo(right, top - bw/2);
                    ctx.lineTo(right, bottom + bw/2);
                }
                ctx.stroke();
            }
